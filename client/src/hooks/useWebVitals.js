...
export function useWebVitals() {
   ...
   * Measures when the largest visible content is painted
   */
  function trackLCP() {
    try {
   ...
   * Measures the delay between first user input and browser response
   */
  function trackFID() {
    try {
      const observer = new PerformanceObserver((list) => {
   ...
   * Measures visual stability - sum of all layout shift scores
   */
  function trackCLS() {
    try {
      let clsValue = 0;
...
function getMetricStyle(rating) {
  const styles = {
    Good: 'color: #10b981; font-weight: bold; background-color: #ecfdf5; padding: 2px 6px; border-radius: 3px;',
    'Needs Improvement': 'color: #f59e0b; font-weight: bold; background-color: #fffbeb; padding: 2px 6px; border-radius: 3px;',
    Poor: 'color: #ef4444; font-weight: bold; background-color: #fef2f2; padding: 2px 6px; border-radius: 3px;',
  };

  return styles[rating] || styles.Good;
}

/**
 * Get metric thresholds for reference
 */
